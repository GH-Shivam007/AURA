...
import { ThreatDataExporter } from '@/utils/threatDataExporter';
...
export const AuraDashboard: React.FC = () => {
  ...
  const dataExporter = ThreatDataExporter.getInstance();
  ...
  useEffect(() => {
    ...
    const generateThreatEvent = (): ThreatEvent => {
      ...
      const accounts = [
        'ACC_CENTRAL_BANK', 'ACC_CRYPTO_EXCHANGE', 'ACC_INVESTMENT_FUND',
        'ACC_PAYMENT_GATEWAY', 'ACC_RETAIL_BANK', 'ACC_CORP_TREASURY',
        'ACC_TRADING_DESK', 'ACC_DIGITAL_WALLET', 'ACC_INSURANCE_CO',
        'ACC_PENSION_FUND', 'ACC_HEDGE_FUND', 'ACC_SOVEREIGN_FUND',
        'ACC_FINTECH_STARTUP', 'ACC_REMITTANCE_SVC', 'ACC_LOAN_PROVIDER'
      ];
      ...
      const destAccount = accounts[Math.floor(Math.random() * accounts.length)];
      ...
      return {
        ...
        timestamp: Date.now(),
        attack_type: isSuspicious ? ['Account Takeover', 'Fraudulent Transfer', 'Money Laundering', 'Identity Theft', 'Payment Fraud'][Math.floor(Math.random() * 5)] : undefined,
    ...
    const interval = setInterval(() => {
      ...
      setThreatEvents(prev => {
        ...
        // Export updated data to JSON format
        dataExporter.exportToJSON(newEvents);
      ...
      if (event.suspicious) {
        const sourceAccount = event.source_account.replace('ACC_', '').replace(/_/g, ' ');
        const destAccount = event.dest_account.replace('ACC_', '').replace(/_/g, ' ');
        toast({
          title: `⚠️ Threat Detected`,
          description: `${event.attack_type}: ${sourceAccount} → ${destAccount}`,
          variant: "destructive"
        });
      }
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              onClick={() => {
                setShowNetworkGraph(!showNetworkGraph);
                setShowHeatmap(false);
              }}
              variant={showNetworkGraph ? "default" : "outline"}
              className="mr-2"
            >
              {showNetworkGraph ? 'Show Globe' : 'Network Graph'}
            </Button>
            <Button
              variant="outline"
              className="mr-4"
        ...
        <div className="flex-1 relative">
          ...
          >
            {showHeatmap ? (
              ...
              <NetworkGraph events={threatEvents} />
            ) : (
              <Canvas
